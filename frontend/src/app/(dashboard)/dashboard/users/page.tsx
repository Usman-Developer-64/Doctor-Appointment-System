...
type UserFilter = 'all' | 'doctor' | 'patient' | 'blocked';
...
export default function UserManagementPage() {
  ...
  const fetchUsers = useCallback(async () => {
    ...
    try {
      ...
      if (activeFilter === 'doctor' || activeFilter === 'patient') {
    ...
    <div className="space-y-8 max-w-5xl mx-auto pb-10">
      ...
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          Monitor user accounts, block malicious profiles, and verify credentials of doctor signups.
        </p>
      </div>
      ...
      <div className="flex flex-wrap gap-2 border-b border-border pb-4">
        {[
          ...
          { label: 'Blocked Accounts', filter: 'blocked' },
        ].map((item) => (
          <Button
            key={item.filter}
              ...
              <tbody className="divide-y divide-border">
                {users.map((item) => {
                  ...
                  return (
                    <tr
                      key={item._id}
                      className={item.isBlocked ? 'bg-red-500/5 opacity-80' : 'bg-card'}
                    >
                      {/* Name Card */}
