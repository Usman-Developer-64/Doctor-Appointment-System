...
export default function ConsultationsPage() {
  ...
  const handleSelectConversation = (
    ...
    socket.on(
      "media-state-changed",
      (data: { type: "video" | "audio"; enabled: boolean }) => {
        if (data.type === "video") {
  ...
  const handleFileUpload = async (
    ...
    reader.onload = () => {
      ...
      handleSendMessage(undefined, {
        ...
        fileName: file.name,
        text:
          type === "image" ? "Sent a photo" : `Sent a document: ${file.name}`,
      });
    };
    ...
    user.role === "patient"
      ? dbDoctors.filter((doc) => {
          ...
          const inAppointments = appointments.some((app) => {
            const doctorId =
              typeof app.doctor === "object" ? app.doctor?._id : app.doctor;
      ...
      {bgNotification && (
        <div className="fixed right-6 top-6 z-50 bg-card border-l-4 border-l-primary border border-border rounded-2xl p-4 shadow-2xl flex items-center gap-3 animate-fade-in w-[320px]">
          ...
          <button
            ...
            className="h-6 w-6 rounded-lg hover:bg-muted flex items-center justify-center text-muted-foreground shrink-0"
          >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md bg-card border border-border rounded-3xl p-6 shadow-2xl z-10 flex flex-col h-[70vh] min-h-[400px]">
            <div className="flex justify-between items-center border-b border-border pb-3 shrink-0">
              ...
              <button
                onClick={() => setShowNewChatModal(false)}
                className="h-7 w-7 hover:bg-muted rounded-lg flex items-center justify-center text-muted-foreground"
              >
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar space-y-1 p-1">
              {isLoadingDoctors ? (
                ...
                <p className="text-center text-xs text-muted-foreground py-10">
                  No verified doctors found in database.
                </p>
              ) : (
                filteredDoctors.map((doc) => {
                  const initials =
                    `${doc.firstName.charAt(0)}${doc.lastName.charAt(0)}`.toUpperCase();
                  return (
                    <button
                      key={doc._id}
                    ...
                    >
                      ...
                      <div>
                        ...
                        <p className="text-[9px] text-muted-foreground mt-0.5">
                          {doc.specialization} • {doc.city || "Available"}
                        </p>
                      </div>
                    </button>
                  );
        ...
        <div className="w-[300px] md:w-[350px] shrink-0 border-r border-border bg-muted/10 flex flex-col justify-between">
          ...
          <div className="p-4 border-b border-border bg-muted/30 flex items-center justify-between shrink-0 h-16">
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <p className="text-[9px] text-muted-foreground capitalize font-semibold tracking-wide">
                  {user.role}
                </p>
              </div>
            </div>
            ...
            {user.role === "patient" && (
              <Button
                ...
                title="New Chat with Database Doctor"
              >
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-1 bg-card">
            {isLoadingChats ? (
              ...
              filteredCustomChats.length === 0 &&
              filteredDirectoryDoctors.length === 0 ? (
              <div className="text-center py-10 text-xs text-muted-foreground leading-relaxed p-4">
                No active consultation chats found.{" "}
                {user.role === "patient" &&
                  "Type any doctor's name in the search box above to start talking directly!"}
              </div>
            ) : (
              <>
                ...
                {filteredAppointments.map((app) => {
                  ...
                  const initials =
                    `${opposite.firstName?.charAt(0) || ""}${opposite.lastName?.charAt(0) || ""}`.toUpperCase();
                    ...
                    {filteredDirectoryDoctors.map((doc) => {
                      ...
                      return (
                        <button
                          ...
                          onClick={() => {
                            // Add to custom chats
                            const exists = customChats.some(
                              (c) => c.roomId === customRoomId,
                            );
                            if (!exists) {
                              const updated = [
                                ...customChats,
              ...
              <div className="bg-card border border-border rounded-3xl p-4 shadow-2xl flex items-center gap-4 animate-bounce">
                <div>
                  <h4 className="font-bold text-xs">
                    Incoming consultation video call
                  </h4>
                  <p className="text-[10px] text-muted-foreground">
                    Answer request to connect streams
                  </p>
                </div>
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center p-8 space-y-4">
              ...
              <div className="space-y-1 max-w-sm">
                ...
                <p className="text-muted-foreground text-xs leading-relaxed">
                  Send and receive messages instantly. Select any doctor or
                  patient appointment session on the left sidebar to start
                  secure consulting.
                </p>
              </div>
            </div>
            ...
            <div className="flex-1 flex flex-col justify-between overflow-hidden h-full">
              ...
              {showPlusMenu && (
                <div className="absolute bottom-16 left-6 z-20 bg-card border border-border rounded-2xl shadow-xl p-3 flex flex-col gap-2 min-w-[150px] animate-fade-in">
                  <button
                    ...
                    className="flex items-center gap-2.5 px-3 py-2 text-xs font-semibold text-muted-foreground hover:text-foreground hover:bg-muted rounded-lg transition-colors"
                  >
              ...
              <div className="p-4 border-b border-border bg-muted/40 flex items-center justify-between shrink-0 h-16">
                <div className="flex items-center gap-3 min-w-0">
                  <div className="h-10 w-10 rounded-full bg-primary/10 flex items-center justify-center text-primary font-bold text-xs relative">
                    ...
                    <span
                      className={`absolute bottom-0 right-0 h-2.5 w-2.5 rounded-full ring-2 ring-card ${peerOnline ? "bg-green-500" : "bg-muted-foreground/30"}`}
                    />
                  </div>
                  <div className="min-w-0">
                ...
                <div className="flex items-center gap-3 shrink-0">
                  {!isCalling && (
                    <Button
                      size="icon"
                      variant="outline"
                      className="h-9 w-9 rounded-full border-primary/20 text-primary hover:bg-primary/5"
              ...
              <div className="flex-1 flex overflow-hidden min-h-0">
                  ...
                  {messages.map((msg, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div
                          className={`max-w-[75%] rounded-2xl p-3 leading-relaxed flex flex-col gap-2 ${
                            isSystem
                          ...
                          {msg.attachmentType === "image" &&
                            msg.attachmentUrl && (
                              <img
                                ...
                                alt="content"
                                className="max-w-[200px] max-h-[160px] rounded-xl object-cover border border-black/10 block mt-1"
                              />
                            )}
                          ...
                          {msg.attachmentType === "document" &&
                            msg.attachmentUrl && (
                              <a
                                ...
                                className="flex items-center gap-2 p-2 bg-black/5 hover:bg-black/10 dark:bg-white/5 dark:hover:bg-white/10 rounded-xl border border-black/5 text-[11px] font-semibold text-current truncate max-w-[220px]"
                              >
                          ...
                          {msg.attachmentType === "voice" &&
                            msg.attachmentUrl && (
                              <audio
                                src={msg.attachmentUrl}
                                controls
                                className="max-w-[220px] h-9 block rounded-lg filter invert dark:invert-0"
                              />
                            )}
                ...
                {isCalling && (
                  <div className="w-[280px] border-l border-border bg-muted/20 p-4 flex flex-col gap-4 shrink-0 min-h-0 animate-fade-in">
                    <h3 className="font-bold text-xs border-b border-border pb-2">
                    ...
                    <div className="flex gap-2 border-t border-border pt-3 mt-auto shrink-0 justify-between">
                      <div className="flex gap-1.5">
                        <Button
                          ...
                          className={`rounded-full h-9 w-9 ${!videoEnabled ? "bg-destructive/10 border-destructive hover:bg-destructive/20 text-destructive" : ""}`}
                        >
                          {videoEnabled ? (
                            <VideoIcon className="h-4 w-4" />
                          ) : (
                        ...
                        <Button
                          ...
                          className={`rounded-full h-9 w-9 ${!audioEnabled ? "bg-destructive/10 border-destructive hover:bg-destructive/20 text-destructive" : ""}`}
                        >
                          {audioEnabled ? (
                      ...
                      <Button
                        variant="default"
                        onClick={cleanupCall}
                        className="bg-red-600 hover:bg-red-700 text-white rounded-xl h-9 text-xs px-4"
                      >
                        End Call
                      </Button>
                    </div>
              ...
              <div className="p-4 border-t border-border flex gap-2 shrink-0 bg-card items-center h-16">
                <Button
                  size="icon"
                  variant="outline"
                  className={`rounded-xl h-11 w-11 shrink-0 ${showPlusMenu ? "bg-muted border-primary" : ""}`}
                  onClick={() => setShowPlusMenu(!showPlusMenu)}
                >
                  <Plus
                    className={`h-5 w-5 transition-transform duration-200 ${showPlusMenu ? "rotate-45" : ""}`}
                  />
                </Button>
                ...
                {isRecordingVoice ? (
                  <div className="flex-1 flex items-center justify-between bg-red-500/10 border border-red-200/50 text-red-600 rounded-xl h-11 px-4 animate-pulse">
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      className="h-8 text-xs text-red-600 border-red-200 hover:bg-red-100"
